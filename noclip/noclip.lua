...
InitNoClip = function()
    ...
    Citizen.CreateThread(function()
        while NoClip.Enabled do Wait(1)
...
NoClipTranslate = function(x,y,z,dist,strafe)
    ...
    local math_rot_x = -math.sin((math.pi / 180) * rot_z) * math.abs(math.cos((math.pi / 180) * rot_x))
    local math_rot_y = math.cos((math.pi / 180) * rot_z) * math.abs(math.cos((math.pi / 180) * rot_x))
    local math_rot_z = math.sin((math.pi / 180) * rot_x)

    local res_x = x + math_rot_x * dist
    local res_y = y + math_rot_y * dist
    local res_z = z + math_rot_z * dist
...
---@return number y Translated Y Coordinate
---@return number z Translated Z Coordinate
NoClipControlTranslation = function(playerPedId,x,y,z)
    local modifier = NoClip.Speed.Default

    -- Speed Modifier
    if IsControlPressed(0, NoClip.Control.LeftShift) then modifier = NoClip.Speed.Fast
    elseif IsControlPressed(0, NoClip.Control.LeftControl) then modifier = NoClip.Speed.Slow end

    -- Translate Coordinates
    if IsControlPressed(0, NoClip.Control.W) then x,y,z = NoClipTranslate(x,y,z,modifier) end
    if IsControlPressed(0, NoClip.Control.S) then x,y,z = NoClipTranslate(x,y,z,0-modifier) end
    if IsControlPressed(0, NoClip.Control.A) then x,y,z = NoClipTranslate(x,y,z,modifier,true) end
    if IsControlPressed(0, NoClip.Control.D) then x,y,z = NoClipTranslate(x,y,z,0-modifier,true) end
    if IsControlPressed(0, NoClip.Control.Spacebar)
    or IsControlPressed(0, NoClip.Control.Q) then z=z+modifier end
    if IsControlPressed(0, NoClip.Control.C)
    or IsControlPressed(0, NoClip.Control.E) then z=z-modifier end
...
da.Dev.NoClip = function(state)
    ...
    if NoClip.Enabled then InitNoClip() end
end
